...
inline static void vcoreiii_bb_writel_hold(struct spi_vcoreiii *priv, u32 value)
{
				   ...
				   const struct spi_mem_op *op)
{
	...
	if ((op->addr.val + op->data.nbytes) < SZ_16M) {
		...
		struct spi_vcoreiii *p = spi_controller_get_devdata(spi->controller);
		u8 __iomem *src = p->read_map + (spi_get_chipselect(spi, 0) * SZ_16M) + op->addr.val;

		if (spi_get_chipselect(spi, 0) != BOOTMASTER_CS)
			return ret;
...
static void vcoreiii_bb_cs_gpio(struct spi_device *spi, bool start)
{
	...
	if (spi_get_csgpiod(spi, 0)) {
		gpiod_direction_output(spi_get_csgpiod(spi, 0), cs_value);
	}
}
...
static void vcoreiii_bb_cs_activate(struct spi_vcoreiii *priv, struct spi_device *spi)
{
	...
	priv->cs_num = spi_get_chipselect(spi, 0);
	...
	if (spi_get_csgpiod(spi, 0) )
		...
		priv->svalue |=
			ICPU_SW_MODE_SW_SPI_CS_OE(BIT(spi_get_chipselect(spi, 0))) |
			ICPU_SW_MODE_SW_SPI_CS(BIT(spi_get_chipselect(spi, 0)));
...
static int vcoreiii_bb_transfer_one_message(struct spi_controller *controller,
					    struct spi_message *msg)
{
	struct spi_vcoreiii *p = spi_controller_get_devdata(controller);
...
static int vcoreiii_bb_probe(struct platform_device *pdev)
{
	...
	controller = spi_alloc_master(&pdev->dev, sizeof(struct spi_vcoreiii));
	if (!controller)
		return -ENOMEM;
	p = spi_controller_get_devdata(controller);
	...
	controller->num_chipselect = MAX_CS;

	controller->transfer_one_message = vcoreiii_bb_transfer_one_message;
	...
	if (res && resource_size(res) >= (SZ_16M*MAX_CS)) {
		...
		if (!IS_ERR(ptr)) {
			p->read_map = ptr;
			controller->mem_ops = &vcoreiii_bb_mem_ops;
	...
	if (err) {
		dev_err(&pdev->dev, "register controller failed: %d\n", err);
		goto fail;
	}
...
static void vcoreiii_bb_remove(struct platform_device *pdev)
{
	struct spi_controller *controller = platform_get_drvdata(pdev);
	struct spi_vcoreiii *priv = spi_controller_get_devdata(controller);
