				 ...
				 struct netlink_ext_ack *extack)
{
	...
	/* Update reference reference configuration */
				     ...
				     struct netlink_ext_ack *extack)
{
	...
	ref_id = zl3073x_input_pin_ref_get(pin->id);
	ref = zl3073x_ref_state_get(zldpll->dev, ref_id);
	*frequency = zl3073x_ref_freq_get(ref);
 ...
 */
static int
zl3073x_dpll_connected_ref_get(struct zl3073x_dpll *zldpll, u8 *ref_id)
{
	struct zl3073x_dev *zldev = zldpll->dev;
	int rc;

	/* Get currently selected input reference */
	rc = zl3073x_dpll_selected_ref_get(zldpll, ref_id);
	if (rc)
		return rc;

	/* If the monitor indicates an error nothing is connected */
	if (ZL3073X_DPLL_REF_IS_VALID(*ref_id)) {
		const struct zl3073x_ref *ref;

		ref = zl3073x_ref_state_get(zldev, *ref_id);
		if (!zl3073x_ref_is_status_ok(zldev, *ref_id))
			*ref_id = ZL3073X_DPLL_REF_NONE;
	}
					...
					struct netlink_ext_ack *extack)
{
	...
	if ((!zldpll->phase_monitor && ref_id != conn_id) ||
	    !zl3073x_ref_is_status_ok(zldev, ref_id)) {
		*phase_offset = 0;
		return 0;
	}
	...
	if (ZL3073X_DPLL_REF_IS_VALID(conn_id) && conn_id != ref_id) {
		const struct zl3073x_ref *conn_ref;
		u32 conn_freq, ref_freq;

		/* Get frequency of connected and given ref */
		conn_ref = zl3073x_ref_state_get(zldev, conn_id);
		conn_freq = zl3073x_ref_freq_get(conn_ref);
			   ...
			   enum dpll_pin_state *state)
{
	...
	if (zldpll->refsel_mode == ZL_DPLL_MODE_REFSEL_MODE_AUTO &&
		 ...
		 * as disconnected
		 */
		if (zl3073x_ref_is_status_ok(zldev, ref)) {
				...
				enum dpll_pin_direction dir, u8 index)
{
	...
	if (dir == DPLL_PIN_DIRECTION_INPUT) {
		...
		is_enabled = zl3073x_dev_out_is_enabled(zldev, out);
	}
