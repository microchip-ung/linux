...
static int policer_conf_set(const struct lan966x *lan966x, u32 pol_ix,
 ...
 ******************************************************************************/
int lan966x_tc_policer_set(const struct lan966x *lan966x, u16 pol_ix,
			   const struct lan966x_tc_policer *pol)
{
	...
	dev_dbg(lan966x->dev, "pol_ix %u rate %u kbps, burst %u bytes\n",
		pol_ix, pol->rate, pol->burst);

	pp.mode = RATE_MODE_DATA;
	pp.pir = pol->rate;
	pp.pbs = pol->burst;
...
int lan966x_tc_port_policer_add(const struct lan966x_port *port,
				const struct lan966x_tc_policer *pol)
{
	const struct lan966x *lan966x = port->lan966x;
	struct qos_policer_conf pp = { 0 };
	...
	netdev_dbg(port->dev,
		   "port %u rate %u kbps, burst %u bytes\n",
		   cp, pol->rate, pol->burst);

	pp.mode = RATE_MODE_DATA;
	pp.pir = pol->rate;
	pp.pbs = pol->burst;

	err = policer_conf_set(lan966x, LAN966X_POL_IX_PORT + cp, &pp);
	if (err)
		return err;
	...
	lan_rmw(ANA_POL_CFG_PORT_POL_ENA_SET(1) |
		...
		lan966x, ANA_POL_CFG(cp));

	return 0;
}

int lan966x_tc_port_policer_del(const struct lan966x_port *port)
{
	const struct lan966x *lan966x = port->lan966x;
	struct qos_policer_conf pp = { 0 };
	u8 cp = port->chip_port;
			      ...
			      const struct mchp_frer_vlan_cfg *const cfg)
{
	...
	return lan966x_vlant_set_mask(lan966x, vid);
}
...
static int lan966x_frer_init(struct lan966x *lan966x)
{
	...
	val = 1000000000000;
	do_div(val, lan966x_clk_period_ps(lan966x));
