...
int sparx5_hsch_l0_get_idx(struct sparx5 *sparx5, int port, int queue)
{
	...
	return (consts->n_hsch_l1_elems * port) +
	       (consts->n_hsch_queues * queue);
}
...
static void sparx5_fp_init(struct sparx5 *sparx5)
{
	...
	for (p = 0; p < consts->n_ports; p++) {
		port = sparx5->ports[p];
		if (!port || (port && ops->is_port_rgmii(port->portno)))
			continue;
...
int sparx5_qos_init(struct sparx5 *sparx5)
{
	...
	ret = sparx5_policer_init(sparx5);
	if (ret)
		return ret;
