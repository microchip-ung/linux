...
#define SPARX5_SDLB_PUP_TOKEN_DISABLE 0x1FFF
#define SPARX5_SDLB_PUP_TOKEN_MAX (SPARX5_SDLB_PUP_TOKEN_DISABLE - 1)
#define SPARX5_SDLB_THRES_MAX 0x3FF
#define SPARX5_SDLB_THRES_HYS_MIN (SPARX5_SDLB_THRES_MAX + 1)
...
static void sparx5_update_u64_counter(u64 *cntr, u32 msb, u32 lsb)
{
	*cntr = (u64)lsb;
	*cntr |= (u64)msb << 32;
}
...
static int sparx5_policer_init(struct sparx5 *sparx5)
{
...
static int sparx5_sdlb_clk_hz_get(struct sparx5 *sparx5)
{
	...
	clk_per_100ps = HSCH_SYS_CLK_PER_SYS_CLK_PER_100PS_GET(
		spx5_rd(sparx5, HSCH_SYS_CLK_PER));
...
static int sparx5_sdlb_pup_token_get(struct sparx5 *sparx5, u32 pup_interval,
				     u64 rate)
{
	u64 clk_hz;

	if (!rate)
		return SPARX5_SDLB_PUP_TOKEN_DISABLE;
...
#define SPARX5_SDLB_GROUP_COUNT 10
static struct sparx5_sdlb_group sdlb_groups[SPARX5_SDLB_GROUP_COUNT] = {
	{ 25000000000, 8192 / 1, 64 }, /*  25 G */
	{ 15000000000, 8192 / 1, 64 }, /*  15 G */
	{ 10000000000, 8192 / 1, 64 }, /*  10 G */
	{  5000000000, 8192 / 1, 64 }, /*   5 G */
	{  2500000000, 8192 / 1, 64 }, /* 2.5 G */
	{  1000000000, 8192 / 2, 64 }, /*   1 G */
	{   500000000, 8192 / 2, 64 }, /* 500 M */
	{   100000000, 8192 / 4, 64 }, /* 100 M */
	{    50000000, 8192 / 4, 64 }, /*  50 M */
	{     5000000, 8192 / 8, 64 }  /*   5 M */
};
...
int sparx5_sdlb_group_get_next(struct sparx5 *sparx5, u32 group, u32 sdlb)
{
	...
	val = spx5_rd(sparx5, ANA_AC_SDLB_XLB_NEXT(sdlb));
...
{
	return sdlb == sparx5_sdlb_group_get_first(sparx5, group);
}

static bool sparx5_sdlb_group_is_last(struct sparx5 *sparx5, u32 group,
				      u32 sdlb)
{
	return sdlb == sparx5_sdlb_group_get_next(sparx5, group, sdlb);
}
...
/* Configure single lb set */
int sparx5_sdlb_group_link(struct sparx5 *sparx5, u32 group, u32 idx, u32 first,
			   u32 next, bool empty)
{
	/* Stop leaking */
	...
	/* Link insertion xlb to next xlb */
...
int sparx5_sdlb_group_get_by_rate(struct sparx5 *sparx5, u32 rate, u32 burst)
{
	...
	for (i = SPARX5_SDLB_GROUP_COUNT - 1; i >= 0; i--) {
		group = &sdlb_groups[i];
...
static int sparx5_sdlb_group_get_by_index(struct sparx5 *sparx5, u32 idx,
					  u32 *group)
{
	u32 itr, next;
	int i;

	for (i = 0; i < SPARX5_SDLB_GROUP_COUNT; i++) {
			   ...
			   u32 frame_size, u32 idx)
{
	...
	max_token = (min_burst > SPARX5_SDLB_PUP_TOKEN_MAX) ?
			    SPARX5_SDLB_PUP_TOKEN_MAX :
			    min_burst;
	group->pup_interval =
	...
	spx5_wr(ANA_AC_SDLB_FRM_RATE_TOKENS_FRM_RATE_TOKENS_SET(
			group->frame_size),
...
/* In hardware, a value of zero for sfid, sgid means disabled - so we
 * make sure the returned id is always above zero.
 */
...
static int sparx5_pool_entry_get_free(struct sparx5_pool_entry *pool,
				       int pool_size)
{
	...
	for (i = 0; i < pool_size; i++) {
		...
		if (e_itr->ref_cnt == 0) {
			e_itr->ref_cnt++;
			return sparx5_pool_idx_to_id(i);
		}
	}
...
static int sparx5_pool_entry_put(struct sparx5_pool_entry *pool, int pool_size,
				 u32 id)
{
	struct sparx5_pool_entry *e;

	e = (pool + sparx5_pool_id_to_idx(id));
	if (e->ref_cnt == 0)
		return -EINVAL;
...
static int sparx5_pool_entry_get(struct sparx5_pool_entry *pool, int pool_size,
				 u32 idx)
{
	struct sparx5_pool_entry *e_itr;
	int i, ret = -ENOSPC;
...
const struct sparx5_psfp_sg sparx5_sg_open = {
	.ipv = 0,
	.cycletime = 1000000,
	.num_entries = 1,
	.gate_state = 1,
	.gate_enabled = 1,
	.gce[0].gate_state = 1,
	.gce[0].interval = 1000000,
	.gce[0].ipv = 0,
	.gce[0].maxoctets = 0,
};

/* Stream filter resource pool */
struct sparx5_pool_entry sparx5_sf_pool[SPARX5_PSFP_SF_NUM];

/* Stream gate resource pool (last entry is reserved for always open gate)*/
struct sparx5_pool_entry sparx5_sg_pool[SPARX5_PSFP_SG_NUM - 1];

/* Service policer resource pool */
struct sparx5_pool_entry sparx5_pol_srv_pool[SPARX5_POL_SRV_NUM];

static int sparx5_psfp_sf_get(void)
{
	return sparx5_pool_entry_get_free(sparx5_sf_pool, SPARX5_PSFP_SF_NUM);
}

static int sparx5_psfp_sf_put(u32 id)
{
	return sparx5_pool_entry_put(sparx5_sf_pool, SPARX5_PSFP_SF_NUM, id);
}

int sparx5_psfp_sg_get(u32 idx)
{
	return sparx5_pool_entry_get(sparx5_sg_pool, SPARX5_PSFP_SG_NUM - 1, idx);
}

int sparx5_psfp_sg_put(u32 id)
{
	return sparx5_pool_entry_put(sparx5_sg_pool, SPARX5_PSFP_SG_NUM - 1, id);
}

static int sparx5_psfp_fm_get(u32 idx)
{
	return sparx5_pool_entry_get(sparx5_pol_srv_pool, SPARX5_POL_SRV_NUM, idx);
}

static int sparx5_psfp_fm_put(u32 id)
{
	return sparx5_pool_entry_put(sparx5_pol_srv_pool, SPARX5_POL_SRV_NUM, id);
}

static int sparx5_sdlb_conf_set(struct sparx5 *sparx5,
				struct sparx5_psfp_fm *fm)
{
	int (*sparx5_sdlb_group_action)(struct sparx5 *, u32, u32);
	...
	/* Configure single sdlb (service policer) */
...
int sparx5_psfp_fm_del(struct sparx5 *sparx5, u32 id)
{
	...
	/* Find the group that this xlb belongs to */
			      ...
			      const struct sparx5_psfp_sf *sf)
{
	...
	spx5_rmw(ANA_AC_TSN_SF_CFG_TSN_SGID_SET(sf->sgid) |
		 ...
		 ANA_AC_TSN_SF_CFG_TSN_STREAM_BLOCK_OVERSIZE_STATE_SET(
		 sf->sblock_osize) |
		 ANA_AC_TSN_SF_CFG_TSN_STREAM_BLOCK_OVERSIZE_ENA_SET(
		 sf->sblock_osize_ena),
		 ANA_AC_TSN_SF_CFG_TSN_SGID | ANA_AC_TSN_SF_CFG_TSN_MAX_SDU |
		 ANA_AC_TSN_SF_CFG_TSN_STREAM_BLOCK_OVERSIZE_STATE |
		 ANA_AC_TSN_SF_CFG_TSN_STREAM_BLOCK_OVERSIZE_ENA,
...
/* Internal priority value to internal priority selector (used by hw) */
static u32 sparx5_psfp_ipv_to_ips(s32 ipv)
{
	return ipv > 0 ? (ipv | BIT(3)) : 0;
}

static inline int sparx5_sgid_get_status(struct sparx5 *sparx5)
{
	return spx5_rd(sparx5, ANA_AC_SG_ACCESS_CTRL);
}

/* Wait for sg config change to complete or timeout. */
#define SPARX5_PSFP_SG_CONFIG_CHANGE_SLEEP 1000
#define SPARX5_PSFP_SG_CONFIG_CHANGE_TIMEO 100000
static int sparx5_sgid_wait_for_completion(struct sparx5 *sparx5)
{
	u32 val;

	return readx_poll_timeout(sparx5_sgid_get_status,
				  sparx5,
				  val,
				  !ANA_AC_SG_ACCESS_CTRL_CONFIG_CHANGE_GET(val),
				  SPARX5_PSFP_SG_CONFIG_CHANGE_SLEEP,
				  SPARX5_PSFP_SG_CONFIG_CHANGE_TIMEO);
}
...
static void sparx5_psfp_sg_config_change(struct sparx5 *sparx5, u32 id)
{
	...
	if (sparx5_sgid_wait_for_completion(sparx5) < 0)
...
static void sparx5_psfp_sg_set(struct sparx5 *sparx5, u32 id,
			      const struct sparx5_psfp_sg *sg)
{
	const struct sparx5_psfp_gce *gce;
	u32 accum_time_interval = 0;
	u32 ips;
	int i;
	...
	spx5_wr(sg->basetime.tv_sec & 0xffffffff, sparx5, ANA_AC_SG_CONFIG_REG_2);

	spx5_rmw(ANA_AC_SG_CONFIG_REG_3_BASE_TIME_SEC_MSB_SET(sg->basetime.tv_sec >> 32) |
	...
	for (i = 0; i < sg->num_entries; i++) {
		...
		spx5_wr(ANA_AC_SG_GCL_GS_CONFIG_IPS_SET(ips) |
				ANA_AC_SG_GCL_GS_CONFIG_GATE_STATE_SET(
					gce->gate_state),
