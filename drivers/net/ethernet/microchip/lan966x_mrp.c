...
static void lan966x_mrp_terminate_ring_test(struct lan966x_mrp *mrp)
{
	...
	if (mrp->p_port) {
		...
		lan966x_mact_learn(lan966x, PGID_MRP, mrp_test_dmac,
...
static void lan966x_mrp_terminate_in_test(struct lan966x_mrp *mrp)
{
	...
	if (mrp->p_port) {
		...
		lan966x_mact_learn(lan966x, PGID_MRP, mrp_in_test_dmac,
				   mrp->p_port->pvid, ENTRYTYPE_LOCKED);
					   ...
					   u32 *slowid)
{
	...
	info.oam_type = 2;
	info.timestamp = 0;
	info.port = BIT(src->chip_port);
	lan966x_port_xmit_impl(skb, &info, src->dev);
				       ...
				       u32 max)
{
	...
	/* Start work to see when loc happens to notify framework */
	if (mrp->lan966x->ana_poll)
		queue_delayed_work(system_wq, &mrp->ring_loc_work,
				   usecs_to_jiffies(mrp->ring_interval));
				     ...
				     u32 max)
{
	...
	/* Start work to see when loc happens to notify framework */
	if (mrp->lan966x->ana_poll)
		queue_delayed_work(system_wq, &mrp->in_loc_rc_work,
				   usecs_to_jiffies(mrp->in_interval));
			   ...
			   const struct switchdev_obj_mrp *mrp)
{
	...
	pr_info("%s: %s %d\n", __FUNCTION__, lan966x->hw_bridge_dev->name,
		mrp->ring_id);
