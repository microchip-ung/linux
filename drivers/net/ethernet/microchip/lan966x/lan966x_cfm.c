				     ...
				     const struct switchdev_obj *obj)
{
	...
	struct afi_slow_inj_alloc_cfg alloc_cfg;
	...
	if (!stop && (mep->afi_id == MEP_AFI_ID_NONE)) {
		...
		if (afi_slow_inj_alloc(mep->port->lan966x->afi_ctrl,
					&alloc_cfg, &mep->afi_id))
			return -EOPNOTSUPP;
	} else {
		/* This is a stop or transmission is ongoing */
		if (mep->afi_id != MEP_AFI_ID_NONE)
			/* Stop any ongoing AFI transmission */
			(void)afi_slow_inj_stop(mep->port->lan966x->afi_ctrl,
						mep->afi_id);
	}
	...
	if (stop) {
		...
		if (mep->afi_id != MEP_AFI_ID_NONE)
			/* AFI must be freed */
			afi_slow_inj_free(mep->port->lan966x->afi_ctrl,
					  mep->afi_id);
		mep->afi_id = MEP_AFI_ID_NONE;
		return 0;
	}
	...
	if (!afi_slow_inj_frm_hijack(mep->port->lan966x->afi_ctrl, mep->afi_id)) {
		if (mep->afi_id != MEP_AFI_ID_NONE)
			/* AFI must be freed */
			afi_slow_inj_free(mep->port->lan966x->afi_ctrl,
					  mep->afi_id);
	...
	afi_slow_inj_start(mep->port->lan966x->afi_ctrl, mep->afi_id, &start_cfg);
