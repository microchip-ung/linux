...
static void lan9645x_vcap_is1_port_keys(struct lan9645x_port *port,
					struct vcap_admin *admin,
					struct vcap_output_print *out)
{
	struct net_device *ndev = lan9645x_port_to_ndev(port);
	...
	val = lan_rd(lan9645x, ANA_VCAP_CFG(port->chip_port));
	out->prf(out->dst, "\n    state: ");
	...
	for (int l = 0; l < admin->lookups; ++l) {
		...
		val = lan_rd(lan9645x,
			     ANA_VCAP_S1_CFG(port->chip_port, l));
...
static void lan9645x_vcap_is2_port_keys(struct lan9645x_port *port,
					struct vcap_admin *admin,
					struct vcap_output_print *out)
{
	struct net_device *ndev = lan9645x_port_to_ndev(port);
...
static void lan9645x_vcap_es0_port_keys(struct lan9645x_port *port,
					struct vcap_admin *admin,
					struct vcap_output_print *out)
{
	struct net_device *ndev = lan9645x_port_to_ndev(port);
	...
	val = lan_rd(lan9645x, REW_PORT_CFG(port->chip_port));
	out->prf(out->dst, "\n    state: ");
			    ...
			    struct vcap_output_print *out)
{
	...
	switch (admin->vtype) {
	case VCAP_TYPE_IS2:
		lan9645x_vcap_is2_port_keys(port, admin, out);
		break;
	case VCAP_TYPE_IS1:
		lan9645x_vcap_is1_port_keys(port, admin, out);
		break;
	case VCAP_TYPE_ES0:
		lan9645x_vcap_es0_port_keys(port, admin, out);
		break;
	default:
