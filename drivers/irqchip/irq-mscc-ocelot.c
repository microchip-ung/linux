...
static void luton_irq_force(struct irq_data *data, struct irq_chip_generic *gc,
			    struct chip_props *p)
{
...
static int ocelot_irq_force(struct irq_data *data, enum irqchip_irq_state which,
			    bool state)
{
	struct irq_chip_generic *gc = irq_data_get_irq_chip_data(data);
	struct irq_domain *d = data->domain;
